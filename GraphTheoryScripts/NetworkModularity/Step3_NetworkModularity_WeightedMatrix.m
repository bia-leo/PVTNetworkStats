...
% adapted by Miranda Chappel-Farley for Weighted Adjacency Matrices using a
% uniform null model from Rick Betzel that is run over a series of resolution parameters.
...
clear i;
% NEED TO MAKE SURE MEDIAN WEIGHTS ARE NOT CALCULATED INCLUDING EXCLUDED
% SUBJECTS. The subjects that need to be excluded are as follows beacon_id
% (preproc number): 
%     Zach scans- 1001(1), 1007(4), 1013(9), 1032(14), 1057(28) 
%     Soyun scans- 1113(2), 1117(3) or in other words in the combined matrix
%     numbers 36 and 37. 
...
out_std = std(out);
out_mean = mean(out);
% NEED TO CALCULATE MEDIAN EDGE WEIGHT OF POSITIVES ONLY - 2STD for
...
gamma = 0:0.05:gamma_range_max; % 5 possible values of gamma, ranging 0, to the median, and up to max edge weight in 0.2 steps
...
for index = 1:length(files)
  ...
  % means indirect paths and their corre- sponding direct connections are more likely to 
  ...
  for i = 1:levelsOfGamma
      Dp(:, :, i) = D(:,:,i)/numPartitions;  % this is for one subject
  end
  ...
  % Current tau/threhsold = .3, though Rick uses .5,  which is a bit high based on Lancichinett and
